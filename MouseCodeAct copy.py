...
import time
import pyautogui
import webbrowser  # Libreria de python para abrir URLs en el navegador
...
while True:
    ...
    if key == 32:  # ESPACIO
        ...
        url_visor_medico = "https://viewer.ohif.org/"
        
        try:
            print(" Conectando con OHIF Viewer...")
            webbrowser.open_new_tab(url_visor_medico)
            print(" OHIF Viewer iniciado correctamente")
            print(" Sistema listo para visualización de imágenes DICOM")
            print(" Control por gestos ACTIVADO")
        except Exception as error_ohif:
            print(f" Error crítico: No se pudo abrir el visor médico")
            print(f"Detalles: {error_ohif}")
        # ==================== FIN DEL BLOQUE ====================
            ...
            if abs(diferencia_y) > 20:  # Umbral para activar scroll
                if diferencia_y > 0:
                    cv2.putText(frame, 'SCROLL UP', (200, 70), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    pyautogui.scroll(100)  # Scroll hacia arriba
                else:
                    cv2.putText(frame, 'SCROLL DOWN', (200, 70), 
                               cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
                    pyautogui.scroll(-100)  # Scroll hacia abajo
